...
# Takes dictionary and writes to a file
# Output file will not be ordered!
#
...
# Simple counting method, requires the key and value indices, also requires a list of possibilities
# Allows user to ignore certain possibilities to ensure the output files are not bloated
#
def count_variations(input_directory_path, output_file_path, key_index, value_index, first_line, header_string, relevant_possibilities):
	...
	# Loop through partioned files directory
	for filename in os.listdir(input_directory_path):
		...
		for row in csv_f:
			if(first_line):
				...
				# Only loop through relevant indices
				for i in range(0,len(relevant_possibilities)):
